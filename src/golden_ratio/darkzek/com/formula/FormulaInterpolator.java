 ...
 * settings of the graph, and slowly morphs them into the target settings the user requested. This
 * does things like makes the points animate towards where they should be when changing sliders, as
 * well as animating colour changes and zooming in and out. Because this is on a seperate thread we
 * need to be careful to shut it down when the window is closed.
 */
        ...
        Platform.runLater(
                () -> {
                    ...
                    if (appliedSettings.rotationPerPoint.getValue()
                        ...
                        if (appliedSettings.rotationPerPoint.getValue()
                                != targetSettings.rotationPerPoint.getValue()) {
                            appliedSettings.rotationPerPoint.setExpression(
                                    targetSettings.rotationPerPoint.getExpression());
                        }
                    }

                    if (appliedSettings.distancePerRotation != targetSettings.distancePerRotation
                            || updateAll) {
                        appliedSettings.distancePerRotation =
                                lerp(
                                        appliedSettings.distancePerRotation,
                                        targetSettings.distancePerRotation,
                                        animationSpeed,
                                        0.001);
                    }
                    if (appliedSettings.sizeIncreasePerPoint != targetSettings.sizeIncreasePerPoint
                            || updateAll) {
                        appliedSettings.sizeIncreasePerPoint =
                                lerp(
                                        appliedSettings.sizeIncreasePerPoint,
                                        targetSettings.sizeIncreasePerPoint,
                                        animationSpeed,
                                        0.001);
                    }
                    if (appliedSettings.defaultSize != targetSettings.defaultSize || updateAll) {
                        appliedSettings.defaultSize =
                                lerp(
                                        appliedSettings.defaultSize,
                                        targetSettings.defaultSize,
                                        animationSpeed * 100,
                                        0.001);
                    }
                    if (appliedSettings.startColor != targetSettings.startColor || updateAll) {
                        appliedSettings.startColor =
                                Helper.lerpColor(
                                        appliedSettings.startColor,
                                        targetSettings.startColor,
                                        animationSpeed);
                    }
                    if (appliedSettings.endColor != targetSettings.endColor || updateAll) {
                        appliedSettings.endColor =
                                Helper.lerpColor(
                                        appliedSettings.endColor,
                                        targetSettings.endColor,
                                        animationSpeed);
                    }
